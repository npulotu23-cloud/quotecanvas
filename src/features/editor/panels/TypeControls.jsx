...
export function TypeControls({ style, overrides, updateOverride, quote, author, onQuoteChange, onAuthorChange, compact = false }) {
  if (compact) {
    return (
  ...
  return (
    <>
      <Section title="Text">
        <div>
          <label className="text-xs text-white/50 uppercase tracking-wide mb-1.5 block">Quote</label>
          <textarea
            value={quote}
            onChange={(e) => onQuoteChange(e.target.value)}
            placeholder="Quote text"
            rows={4}
            className="w-full border border-[#2a2a2a] rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-[#FFB547]/50 resize-none"
            style={{ backgroundColor: '#141414', color: '#ffffff', WebkitTextFillColor: '#ffffff' }}
          />
        </div>
        <div>
          <label className="text-xs text-white/50 uppercase tracking-wide mb-1.5 block">Speaker</label>
          <input
            value={author}
            onChange={(e) => onAuthorChange(e.target.value)}
            placeholder="Speaker name"
            className="w-full border border-[#2a2a2a] rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-[#FFB547]/50"
            style={{ backgroundColor: '#141414', color: '#ffffff', WebkitTextFillColor: '#ffffff' }}
          />
        </div>
      </Section>
